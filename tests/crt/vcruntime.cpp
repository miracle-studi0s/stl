...
void c_vcruntime::strncpy(char* _dst, const char* _src, int _n)
{
	int i = 0;
...
{
	wchar_t leading_integer[31];  // 63 instead of 31 for 64-bit systems
	wchar_t trailing_decimal[31];  // 63 instead of 31 for 64-bit systems
...
int c_vcruntime::strncmp(const char* s1, const char* s2, register int n)
{
...
int c_vcruntime::rfind(const char* s, const char* s2)
{
	const int ln1 = this->strlen(s);
	const int ln2 = this->strlen(s2);
	...
	if (text)//Проверяем не пустой ли ввод
		if (text + beg)//Проверяем существование в тексте позиции beg
			if (0 < len)//Проверяем корректность параметров конец должен быть больше начала
				if ((sub = (char*)allocation::malloc(1 + len)))//Если end превосходит последнюю
...
int c_vcruntime::w2c(const wchar_t* src, char* dest, int dest_len)
{
	int i;
	wchar_t code;
